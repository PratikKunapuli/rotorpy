...
class DebugController(object):
    ...
    def __init__(self, vehicle_params):
        ...
        self.hover_thrust = vehicle_params['mass'] * 9.81
        print("Running Dumb Controller")
    ...
    def update(self, t, state, flat_output):
        ...
        # We're going to set the thrust to be the defualt hover thrust but the orientation to be some constant value.

        # Set the thrust to be the hover thrust
        cmd_thrust = self.hover_thrust

        # Set the orientation to be a constant value
        desired_roll = np.pi/4
        desired_pitch = 0
        desired_yaw = 0
        cmd_q = Rotation.from_euler('ZYX', [desired_yaw, desired_pitch, desired_roll]).as_quat()

        print("Commanded thrust: ", cmd_thrust)
        print("Commanded orientation: ", cmd_q)
