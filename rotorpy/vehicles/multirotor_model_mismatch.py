...
class MultirotorModelMismatch(object):
    """
    ...
    def __init__(self, quad_params, initial_state = {'x': np.array([0,0,0]),
        ...
        """
        # Assumed parameters
        if assumed_quad_params is None:
        ...
        self.assumed_mass    = self.assumed_quad_params['mass'] # kg
        ...
        self.assumed_inertia = np.array([[self.assumed_quad_params['Ixx'], self.assumed_quad_params['Ixy'], self.assumed_quad_params['Ixz']],
                                         [self.assumed_quad_params['Ixy'], self.assumed_quad_params['Iyy'], self.assumed_quad_params['Iyz']],
                                         [self.assumed_quad_params['Ixz'], self.assumed_quad_params['Iyz'], self.assumed_quad_params['Izz']]])
        ...
        self.assumed_k = self.assumed_quad_params['k_m'] / self.assumed_quad_params['k_eta']
        self.assumed_k_eta = self.assumed_quad_params['k_eta']
        self.assumed_f_to_TM = np.vstack((np.ones((1,self.num_rotors)),np.hstack([np.cross(self.rotor_pos[key],np.array([0,0,1])).reshape(-1,1)[0:2] for key in self.rotor_pos]), (self.assumed_k * self.rotor_dir).reshape(1,-1)))
        self.assumed_TM_to_f = np.linalg.inv(self.assumed_f_to_TM)
    ...
    def step(self, state, control, t_step):
        ...
        # These are allocated under model mismatch if assumed_quad_params is not None.
        cmd_rotor_speeds = self.get_cmd_motor_speeds(state, control)
    ...
    def _s_dot_fn(self, t, s, cmd_rotor_speeds):
        ...
        w = state['w']
        w_hat = MultirotorModelMismatch.hat_map(w)
    ...
    def compute_body_wrench(self, body_rates, rotor_speeds, body_airspeed_vector):
        ...
        local_airspeeds = body_airspeed_vector[:, np.newaxis] + MultirotorModelMismatch.hat_map(body_rates)@(self.rotor_geometry.T) 
        ...
        if self.aero:
            ...
            D = -MultirotorModelMismatch._norm(body_airspeed_vector)*self.drag_matrix@body_airspeed_vector
            ...
            M_flap = -self.k_flap*rotor_speeds*((MultirotorModelMismatch.hat_map(local_airspeeds.T).transpose(2, 0, 1))@np.array([0,0,1])).T
        else:
            D = np.zeros(3,)
        ...
        M_force = -np.einsum('ijk, ik->j', MultirotorModelMismatch.hat_map(self.rotor_geometry), T+H)
    ...
    def get_cmd_motor_speeds(self, state, control):
        ...
        if self.control_abstraction == 'cmd_motor_speeds':
            ...
            cmd_moment = self.assumed_inertia@wdot_cmd
            ...
            F_des = self.assumed_mass*(a_cmd + np.array([0, 0, self.g]))
            ...
            cmd_moment = self.assumed_inertia @ (-self.kp_att*att_err - self.kd_att*state['w']) + np.cross(state['w'], self.assumed_inertia@state['w'])
            ...
            cmd_moment = self.assumed_inertia @ (-self.kp_att*att_err - self.kd_att*state['w']) + np.cross(state['w'], self.inertia@state['w'])
        ...
        cmd_motor_forces = self.assumed_TM_to_f @ TM                                                # Convert to cmd_motor_forces from allocation matrix
        cmd_motor_speeds = cmd_motor_forces / self.assumed_k_eta                                    # Convert to motor speeds from thrust coefficient. 
